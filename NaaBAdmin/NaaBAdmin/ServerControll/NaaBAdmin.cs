//namespace NaaBAdmin.ServerControll
using System;
using System.Text;
...
{
    public partial class NaaBAdmin : BaseScript
    {
            ...
            : base()
        {
            ...
            WriteLog.Info("NaaBAdmin Is Starting...");
            ...
            WriteLog.Info("Reading Configs...");
            if (!System.IO.Directory.Exists(ConfigValues.ConfigPath + @""))
                System.IO.Directory.CreateDirectory(ConfigValues.ConfigPath + @"");

            if (!File.Exists(ConfigValues.ConfigPath + @"Settings.txt"))
                CFG_CreateConfig();
            ...
            if (!System.IO.File.Exists(ConfigValues.ConfigPath + @"BlackNickNames.txt"))
                System.IO.File.WriteAllLines(ConfigValues.ConfigPath + @"BlackNickNames.txt", new string[]
                    {
                        "WriteBelow:",
                    });

            if (!System.IO.File.Exists(ConfigValues.ConfigPath + @"BlackClanTags.txt"))
                System.IO.File.WriteAllLines(ConfigValues.ConfigPath + @"BlackClanTags.txt", new string[]
                    {
                        "WriteBelow:",
                    });
        ...
        public void CMDS_OnPlayerKilled(Entity player, Entity inflictor, Entity attacker, int damage, string mod, string weapon, Vector3 dir, string hitLoc)
        {
            ...
            if (settings_enable_spree_messages)
            {
                ...
                if (mod == "MOD_HEAD_SHOT")
                    Utilities.RawSayAll(ServerControll.BotName + Lang_GetString("Spree_Headshot").Format(new Dictionary<string, string>()
                    {
                ...
                if (attacker_killstreak == 5)
                    Utilities.RawSayAll(ServerControll.BotName + Lang_GetString("Spree_Kills_5").Format(new Dictionary<string, string>()
                    {
                        {"<attacker>", attacker.Name}
                    }));

                if (attacker_killstreak == 10)
                    Utilities.RawSayAll(ServerControll.BotName + Lang_GetString("Spree_Kills_10").Format(new Dictionary<string, string>()
                    {
                        {"<attacker>", attacker.Name}
                    }));
                ...
                switch (weapon)
                {
                    ...
                    case "destructible_toy":
                        Utilities.RawSayAll(ServerControll.BotName + Lang_GetString("Spree_Explosivekill").Format(new Dictionary<string, string>()
                        {
                            {"<victim>", player.Name}
                        }));
                        break;
                    case "trophy_mp":
                        Utilities.RawSayAll(ServerControll.BotName + Lang_GetString("Spree_Trophykill").Format(new Dictionary<string, string>()
                        {
                            {"<attacker>", attacker.Name},
                            {"<victim>", player.Name}
                        }));
                        break;
                    case "knife":
                        Utilities.RawSayAll(ServerControll.BotName + Lang_GetString("Spree_KnifeKill").Format(new Dictionary<string, string>()
                        {
                            {"<attacker>", attacker.Name},
                            {"<victim>", player.Name}
                        }));
                ...
                if (victim_killstreak >= 5)
                    Utilities.RawSayAll(ServerControll.BotName + Lang_GetString("Spree_Ended").Format(new Dictionary<string, string>()
                    {
                        {"<attacker>", attacker.Name},
                        {"<victim>", player.Name},
        ...
        {
            //check if bad name
            foreach (string identifier in System.IO.File.ReadAllLines(ConfigValues.ConfigPath + @"BlackListNickName.txt"))
                if (player.Name == identifier)
                {
                    CMD_tmpban(player, "Your NickName Is In The BlackList!");
                    Utilities.RawSayAll("^1" + player.Name + "^7 Was TempBanned! Reason: ^5Bad NickName");
                    return;
                }

            //check if bad clantag
            foreach (string identifier in System.IO.File.ReadAllLines(ConfigValues.ConfigPath + @"BlackListClanTag.txt"))
                if (player.GetClantag() == identifier)
                {
                    CMD_tmpban(player, "Your ClanTag Is In The BlackList!");
                    Utilities.RawSayAll("^1" + player.Name + "^7 Was TempBanned! Reason: ^5Bad ClanTag");
                    return;
                }
            AfterDelay(1100, () =>
            {
                ...
                */
                player.SpawnedPlayer += new Action(() =>
                {
                    if (player.GetField<string>("sessionteam") == "axis")
                    {
                        ...
                        {
                                pl.SetClientDvar("cg_teamcolor_axis", "0 1 1 1");
                                pl.SetClientDvar("cg_teamcolor_allies", "1 0.498 0 1");
                            }
                    }
                    else if (player.GetField<string>("sessionteam") == "allies")
                            {
                                ...
                                {
                                    pla.SetClientDvar("cg_teamcolor_allies", "1 0.498 0 1");
                                    pla.SetClientDvar("cg_teamcolor_axis", "0 1 1 1");
                                }
                            }
                        });
                    }
        ...
        private void ColorScoreboard(Entity player, string mapname)
       {
           ...
           {
               case "mp_aground_ss":
                   player.SetClientDvar("g_ScoresColor_Axis", "0 1 1 1");
                   player.SetClientDvar("g_ScoresColor_Allies", "1 0.498 0 1");
